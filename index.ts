...
import type * as yetfTypes from '@cityssm/mpac-yetf/types'

const argv = yargs(hideBin(process.argv))
  .usage('Usage: $0 --inputFile [inputFile] --outputFile [outputFile]')
  .command(
    '--inputFile [inputFile] --outputFile [outputFile]',
    'Process the MPAC YETF',
    (yargs) => {
      return yargs
...
try {
  ...
  await parseYetf(inputFile, {
    ...
    callbacks: {
      ...
      CC: (record: yetfTypes.FormattedYetfRecordCC) => {
        database
          ...
          .run(
            ...
            record.characterOfConstruction,
            record.characterOfConstructionDescription,
            record.characterOfConstructionFramingDescription,
            record.characterOfConstructionFloorDescription,
            record.characterOfConstructionRoofDescription,
            record.characterOfConstructionWallsDescription,
            record.quality,
            record.shape,
            record.yearBuilt,
            record.yearBuiltCode,
            record.yearBuiltCodeName,
            record.condition,
            record.conditionName,
            record.conditionRankingOutOf5,
            record.fullStoreys,
            record.partStoreys,
            record.partStoreysName,
            record.heightEffectiveYearBuilt,
            record.heightFeet,
            record.effectiveYearBuilt,
            record.split,
            record.splitName,
            record.grossArea,
            record.grossAreaSquareFeet,
            record.totalBasementArea,
            record.totalBasementAreaSquareFeet,
            record.finishedBasementArea,
            record.finishedBasementAreaSquareFeet,
            record.basementFinish,
            record.basementFinishName,
            record.fullBaths,
            record.halfBaths,
            record.numberOfBedrooms,
            record.numberOfFireplaces,
            record.heatingType,
            record.heatingTypeName,
            record.airConditioning,
            record.garageType,
            record.garageTypeName,
            record.garageSpaces,
            record.structureCode,
            record.structureCodeClass,
            record.structureCodeName
          )
      },
      DD: (record: yetfTypes.FormattedYetfRecordDD) => {
        database
          .prepare(
            `insert into DD (
              rollNumber, sequenceNumber,
          ...
          .run(
            ...
            record.textureName,
            record.soilClass,
            record.soilClassPointsRemainingMin,
            record.soilClassPointsRemainingMax,
            record.climaticZone,
            record.woodedAcreage,
            record.orchardAcreage
          )
      },
      GG: (record: yetfTypes.FormattedYetfRecordGG) => {
        database
          .prepare(
            `insert into GG (
              rollNumber, sequenceNumber,
          ...
          .run(
            ...
            record.occupancyStatusName,
            record.religion,
            record.schoolSupport,
            record.schoolSupportName,
            record.residencyCode,
            record.residencyCodeDescription,
            record.citizenship,
            record.designatedRatepayerCode,
            record.yearOfBirth,
            record.monthOfBirth,
            record.frenchLanguageEducationRights
          )
      },
      HH: (record: yetfTypes.FormattedYetfRecordHH) => {
        database
          .prepare(
            `insert into HH (
      ...
      JJ: (record: yetfTypes.FormattedYetfRecordJJ) => {
        database
          .prepare(
            `insert into JJ (
      ...
      KK: (record: yetfTypes.FormattedYetfRecordKK) => {
        database
          .prepare(
            `insert into KK (
              rollNumber,
      ...
      MM: (record: yetfTypes.FormattedYetfRecordMM) => {
        database
          .prepare(
            `insert into MM (
          ...
          .run(
            record.rollNumber,
            record.sequenceNumber,
            record.commentsSiteDimensions
          )
      },
      PA: (record: yetfTypes.FormattedYetfRecordPA) => {
        database
          .prepare(
            `insert into PA (
              rollNumber, sequenceNumber,
          ...
          .run(
            ...
            record.realtyTaxQualifier,
            record.realtyTaxQualifierClass,
